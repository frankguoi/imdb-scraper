...
"""
This code retrives data from all the movies from the IMDb Top 1000 and stores them into files
"""
...
top_urls = ["https://www.imdb.com/search/title/?groups=top_1000&count=250",
            "https://www.imdb.com/search/title/?groups=top_1000&count=250&start=251",
            "https://www.imdb.com/search/title/?groups=top_1000&count=250&start=501",
            "https://www.imdb.com/search/title/?groups=top_1000&count=250&start=751"]
for top_url in top_urls:
    top_page = requests.get(top_url)
    top_soup = BeautifulSoup(top_page.content, 'html.parser')
    movie_headers = top_soup.find_all('h3', class_='lister-item-header')
    movie_ids = []
    for elm in movie_headers:
        mid = re.search("title/(.*)/", elm.find('a')['href']).group(1)
    ...
    for id in movie_ids:
        ...
        genres_dict = {'Action': 1, 'Adventure': 2, 'Animation': 3, 'Biography': 4, 'Comedy': 5, 'Crime': 6,
                       'Documentary': 7, 'Drama': 8, 'Family': 9, 'Fantasy': 10, 'Film-Noir': 11, 'Game-Show': 12,
                       'History': 13, 'Horror': 14, 'Music': 15, 'Musical': 16, 'Mystery': 17, 'News': 18,
                       'Reality-TV': 19, 'Romance': 20, 'Sci-Fi': 21, 'Sport': 22, 'Talk-Show': 23, 'Thriller': 24,
                       'War': 25, 'Western': 26}
        ...
        """
        movie_data = {'mid': mid, 'title': title, 'release_date': release_date, 'summary': summary, 'directors': directors, 
